...
namespace App\Filament\Admin\Resources\Bans;
...
use App\Filament\Admin\Resources\Bans\Pages\ListBans;
use App\Filament\Admin\Resources\Bans\Pages\ViewBan;
use App\Filament\Admin\Resources\Bans\RelationManagers\NotesRelationManager;
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('created_at')->label('Issued')->date()->sortable(),
                TextColumn::make('period_start')->label('Started')->since()->description(fn ($record) => $record->period_start?->toPanelDateTime()),
                TextColumn::make('period_finish')->label('Ends')->description(fn ($record) => $record->period_finish ? $record->period_finish->toPanelDateTime() : 'Permanent')->getStateUsing(fn ($record) => $record->period_finish ? $record->period_finish->diffForHumans() : 'Never')->sortable(),
