...
class Controller_Sso_Auth extends Controller_Master {
    ...
    private function security() {
        // Are we overriding?
        if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_override", null) != null) {
            $this->_actual_account = ORM::factory("Account", Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_override"));

            if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid", null) == null) {
                Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_cid", $this->_actual_account->id);
                Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_override");
            }
        }

        // Are we logged in?
        if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid", null) != null) {
            $this->_current_account = ORM::factory("Account", Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid"));
            $this->_data["_account"] = $this->_current_account;
        }
        
        // Requesting a new token, or using old one?
        if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_token", null) != null && $this->request->query("token") == null) {
            $token = Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_token");
        ...
        if (!$this->_current_token->loaded()) {
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_token");
            return false;
        }
    ...
    public function action_display() {
        ...
        if (!Helper_Account_Main::check_login_status()) {
            ...
            $SSO = new SSO("CORE", URL::site("/sso/auth/display", "http"), false, (Kohana::$environment === Kohana::DEVELOPMENT ? "http://dev.vatsim-uk.co.uk/ALawrence/core.vatsim-uk.co.uk/sso/auth/preLogin" : ""));
            $details = $SSO->member;
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_cid", $details->cid);
        }

        // Get the account details
        $account = ORM::factory("Account", Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid"));
    ...
    public function action_override() {
        ...
        // KH or AL?
        if (!in_array(Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid"), array(980234, 1010573))) {
            $this->redirect("sso/auth/error");
            return;
        }
    ...
    public function process_override() {
        ...
        // Override!
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_override", Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid"));
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_cid", $ovrAccount->id);
    ...
    public function action_preLogin() {
        ...
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_token", $this->_current_token->token);

        // Has this member logged in before? Are we remembering them?
        if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid", null) != null) {
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_fast_login", true);
            $this->process_login();
            return;
        } else {
    ...
    public function action_logout() {
        ...
        if ($returnURL == null || $ssoKey == null) {
            $returnURL = URL::site("/sso/auth/display", "http");
            $ssoKey = "CORE";
        }

        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("logout_url", $returnURL);
    ...
    public function process_logout() {
        $returnURL = Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("logout_url");

        // Result?
        if ($this->request->post("processlogout") == 1) {
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_cid");
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_token");
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_fast_login");
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_login_sls");
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_password_grace");
            Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->destroy();
        }
    ...
    protected function process_login() {
        ...
        if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid", null) != null && Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_fast_login", null) != null) {
            $cid = Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid");
        } else {
            ...
            try {
                ...
                Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_login_sls", true);
                Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_password_grace", gmdate("Y-m-d H:i:s", strtotime("+2 hours")));
            }
        }
        ...
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_cid", $cid);
    ...
    public function process_checkpoint() {
        ...
        if ($this->_current_account->security->loaded() || $this->_current_account->security->find()->loaded()) { // Staff
            ...
            try {
                if (!Vatsim::factory("autotools")->authenticate(Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_cid"), $this->request->post("password"))) {
        ...
        // Extra security is valid!
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->set("sso_checkpoint", true);
        $this->postLoginChecks();
    }
    ...
    private function postLoginChecks() {
        ...
        if ($this->_current_account->security->loaded()){
            ...
            if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_login_sls", null) == null) {
                if (Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_password_grace", null) == null || strtotime(Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_password_grace")) < time()) {
        ...
        /* if(Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_fast_login", null) != null && Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->get("sso_checkpoint", null) == null){
        ...
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_fast_login");
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_checkpoint");
        Session::instance(ORM::factory("Setting")->getValue("system.session.type"))->delete("sso_override");
