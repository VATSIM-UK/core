<x-markdown>
    ...
    In the absence of local ATC, area and approach controllers provide a top-down service at airfields in their area of
    responsibility. You should request clearance, pushback, taxi, etc., as if there were an aerodrome controller online.

    ### Are all airfields covered top-down?

    Controllers provide top-down service at all airfields with a permanent Tower or Approach facility in the real world,
    including airports [outside of controlled
    airspace](https://www.skybrary.aero/index.php/Controlled_Airspace#:~:text=SKYbrary%20Wiki,-If%20you%20wish&text=Controlled%20airspace%20is%20an%20airspace,accordance%20with%20the%20airspace%20classification.).

    When busy, controllers may downgrade service at quieter airfields. You **must** contact the controller before
    departure to check the service provided.

    If you are departing from an airfield outside of controlled airspace and intending to join in-flight, ensure you
    obtain a joining clearance beforehand.

    ### How do I know who to call?

    When there is no local ATC, contact the first listed controller below that is online (left to right priority).
    Callsigns may vary, so always check the frequency.

    You can also refer to the sector maps in the next section.

    #### Manchester (EGCC)
    > MAN_SE_CTR (134.430) → MAN_E_CTR (133.800) → MAN_CTR (133.200) → LON_N_CTR (133.705) → LON_NW_CTR (135.580) →
    LON_CTR (127.830)
