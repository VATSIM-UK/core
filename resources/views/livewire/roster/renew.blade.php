        ...
        <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-[480px]">
            <div class="bg-white px-4 py-2 shadow space-y-6 sm:rounded-lg sm:px-12">
                ...
                @else
                    ...
                    <p>Before you do this, please take a read through what has changed in the Division and Procedurally
                        whilst you've been gone!</p>
                @endif
            </div>
        </div>
        @if ($canReactivate)
            <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-[480px]">
                <div class="bg-white px-4 py-2 shadow space-y-6 sm:rounded-lg sm:px-12">
                    ...
                    <button x-bind:disabled="{{ $this->reactivateButtonDisabled }}"
                        class="p-2 disabled:opacity-25 bg-brand text-white rounded-lg shadow"
                        wire:click="nextPage">Reactivate</button>
                    </p>
                    @if ($notifications->count() > 0)
                            ...
                            @foreach ($notifications as $notification)
                                <div class="text-left" x-data="{ expanded: false }">
                                    <span x-on:click="expanded = ! expanded"
                                        class="text-md font-semibold hover:cursor-pointer">{{ $notification['title'] }}</span>
                                    <button x-on:click="expanded = ! expanded">⬇️</button>
                                    <div x-show="expanded" x-collapse>
                                        <p>{{ $notification['body'] }}</p>
                                        @if ($notification['link'])
                                            <a href="{{ $notification['link'] }}" target="_blank"
                                                lass="text-blue-500 hover:cursor-pointer">Read more</a>
                                        @endif
                                        <p class="text-blue-900 cursor-pointer"
                                            wire:click="markNotificationRead({{ $notification['id'] }}, {{ $notification['id'] }})">
                                            Mark read</p>
                                    </div>
                                </div>
                            @endforeach
...
@elseif ($page === 2)
    <div class="mt-2 sm:mx-auto sm:w-full sm:max-w-[480px]">
        <div class="bg-white px-4 py-2 shadow space-y-6 sm:rounded-lg">
            ...
            <div class="mt-6">
                <p>Once you are added back onto the roster you must maintain a minimum of 3 hours controlling any UK
                    position
                    within a calendar quarter e.g. January -> March.</p>
                <p>Click the button below to add yourself back onto the roster.</p>
                <button class="mt-2 p-2 bg-brand text-white rounded-lg shadow" x-on:click="$wire.reactivate()">Add to
                    roster</button>
            </div>
        </div>
    </div>
