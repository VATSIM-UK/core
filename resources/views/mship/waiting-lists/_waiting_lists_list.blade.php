                    ...
                    <tbody>
                        @foreach ($waitingListAccounts as $waitingListAccount)
                            <tr>
                                ...
                                @if ($department === \App\Models\Training\WaitingList::ATC_DEPARTMENT)
                                    @if (
                                        $waitingListAccount->waitingList->isAtcList() &&
                                            ($waitingListAccount->waitingList->feature_toggles['display_on_roster'] ?? true))
                                        <td>
                                            @if ($waitingListAccount->account->onRoster())
                                                <img src="{{ asset('images/tick_mark_circle.png') }}" width="20"
                                                    alt="Tick">
                                            @else
                                                <img src="{{ asset('images/cross_mark_circle.png') }}" width="20"
                                                    alt="Cross">
                                            @endif
                                        </td>
                                    @else
                                ...
                                <td>
                                    <button class="btn btn-danger" type="button" data-toggle="modal"
                                        data-target="#leaveConfirmModal"
                                        onclick="document.getElementById('leaveConfirmForm').action='{{ route('mship.waiting-lists.self-remove', ['waitingList' => $waitingListAccount->waitingList]) }}'">Leave
                                        Waiting List</button>
                                </td>
                            </tr>
                        @endforeach
                            ...
                            <tr>
                                ...
                                <td>
                                    @if (!$waitingList->isAtCapacity())
                                        <form
                                            action="{{ route('mship.waiting-lists.self-enrol', ['waitingList' => $waitingList]) }}"
                                            method="POST">
                                    ...
                                    @else
                                        <form>
                                            <button class="btn btn-danger" disabled type="submit">Waiting List
                                                Full</button>
                                        </form>
                                    @endif
                                </td>
