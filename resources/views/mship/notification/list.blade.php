						...
						<div class="row">
							<div class="col-md-offset-2 col-md-8">
								@foreach ($unreadNotifications as $notice)
									<div
										class="panel panel-{{ $notice->status == \App\Models\Sys\Notification::STATUS_MUST_ACKNOWLEDGE ? 'danger' : ($notice->status == \App\Models\Sys\Notification::STATUS_IMPORTANT ? 'warning' : 'default') }}">
										<div class="panel-heading" role="tab" id="heading<?= $notice->id ?>">
											<h4 class="panel-title">
												<a data-toggle="collapse" data-parent="#unreadNotifications" href="#collapse<?= $notice->id ?>"
													aria-expanded="false" aria-controls="collapse<?= $notice->id ?>">
													[Effective: {{ $notice->effective_at }}] {{ $notice->title }}

													@if ($notice->status == \App\Models\Sys\Notification::STATUS_MUST_ACKNOWLEDGE)
														<span class="label label-danger">Must Acknowledge before
															continuing</span>
										...
										<div id="collapse<?= $notice->id ?>" class="panel-collapse collapse" role="tabpanel"
											...
											<div class="panel-body">
												...
												<form action="{{ route('mship.notification.acknowledge', $notice->id) }}" method="POST"
													...
													@elseif($notice->status == \App\Models\Sys\Notification::STATUS_IMPORTANT)
														<div class="form-group">
															<div class="col-sm-offset-5 col-sm-7">
																<button type="submit" class="btn btn-warning">Mark
																	Read</button>
															</div>
														</div>
													@else
							...
							<div class="col-md-offset-2 col-md-8">
								@foreach ($readNotifications as $notice)
									<div
										...
										<div class="panel-heading" role="tab" id="heading<?= $notice->id ?>">
											<h4 class="panel-title">
												<a data-toggle="collapse" data-parent="#readNotifications" href="#collapse<?= $notice->id ?>"
													aria-expanded="false" aria-controls="collapse<?= $notice->id ?>">
													[Effective: {{ $notice->effective_at }}] {{ $notice->title }}
												</a>
											</h4>
