...
@section('content')
	<div class="panel panel-ukblue">
		...
		<div class="panel-body">
			...
			<p>
				When you are logged into systems within VATSIM UK, as standard we use your <strong>primary</strong> email
				address
				when sending you important emails from that service.
			</p>
			<p>
				If you wish for <em>some</em> services to dispatch emails to an alternative address <strong>instead of your
					primary email</strong>
				you can specify that using the matrix below.
			</p>
			<p>
				<strong>Please note:</strong> you may only have <em>one</em> email address assigned to a specific system.
				Once that email address
			...
			<form action="{{ route('mship.manage.email.assignments') }}" method="POST" class="form-horizontal">
							...
							<tr>
								...
								@foreach ($userSecondaryVerified as $email)
									<td>
										<div class="radio">
											<label>
												<input type="radio" name="assign_{{ $um['sso_system']->id }}" value="{{ $email->id }}"
													{{ $email->id == $um['assigned_email_id'] ? "checked='checked'" : '' }}>
												Assign
											</label>
										</div>
									</td>
				...
				<div class="form-group">
					<div class="col-sm-offset-5 col-sm-7">
						<button type="submit" class="btn btn-default" name="processassignments" value="new_email_add">Save
							Assignments
						</button>
					</div>
				</div>
