...
{{-- Deprecated: the classic dashboard is retained for direct access only. --}}
@section('content')
							...
							<div class="row">
								@forelse($pluginKeys as $key)
									<div class="col-xs-6 row-text-contain" style="padding-bottom: 20px;">
										...
										<strong>CREATED</strong>:
										<a class="tooltip_displays" href="#" data-toggle="tooltip" title="{{ $key->created_at }}">
											<em>{{ \Carbon\Carbon::createFromTimeString($key->created_at)->diffForHumans() }}</em>
										</a>
										<br />
										<strong>EXPIRES</strong>:
										<a class="tooltip_displays" href="#" data-toggle="tooltip" title="{{ $key->expires_at }}">
