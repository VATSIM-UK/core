					...
					@if ($application->exists)
						@include('components.html.panel_open', [
							'title' =>
								...
								($application->facility ? ' - ' . $application->facility->name : ''),
					...
					<ul class="nav nav-pills nav-stacked">
						...
						@else
							<li role="presentation" class="active">
								<a
									href="{{ route('visiting.application.start', [isset($applicationType) ? $applicationType : $application->type]) }}">Stage
									1 - T&amp;C Acceptance</a>
							</li>
						@endif
						...
						@can('select-facility', $application)
							<li role="presentation" {!! Route::is('visiting.application.facility') ? 'class="active"' : '' !!}>
								<a href="{{ route('visiting.application.facility', [$application->public_id]) }}">Stage 2 -
									Facility Selection</a>
							</li>
						@else
							<li role="presentation" class="disabled">
							...
							@can('add-statement', $application)
								<li role="presentation" {!! Route::is('visiting.application.statement') ? 'class="active"' : '' !!}>
									<a href="{{ route('visiting.application.statement', [$application->public_id]) }}">Stage 3 -
										Personal Statement{{ $application->statement_required ? '' : ' (Not Required)' }}</a>
								</li>
							@else
								<li role="presentation" class="disabled">
									<a href="#">Stage 3 - Personal
										Statement{{ $application->statement_required ? '' : ' (Not Required)' }}</a>
								</li>
								@endif

								@can('submit-application', $application)
									<li role="presentation" {!! Route::is('visiting.application.submit') ? "class='active'" : '' !!}>
										<a href="{{ route('visiting.application.submit', [$application->public_id]) }}">Stage 4 -
											Submission</a>
									</li>
								@else
									<li role="presentation" class="disabled">
									...
									@else
										...
										@can('withdraw-application', $application)
											<li role="presentation" class="text-center" {!! Route::is('visiting.application.withdraw') ? "class='active'" : '' !!}>
												<a href="{{ route('visiting.application.withdraw', [$application->public_id]) }}"
													class="label label-danger label-md">Withdraw Application</a>
											</li>
											@endif
