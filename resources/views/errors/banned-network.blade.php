@extends('layout', ['shellOnly' => true])

@section('content')
	<h1>You are currently suspended from the VATSIM Network.</h1>
	<p>As your account is suspended by the central VATSIM.net team, you will need to follow the instructions listed on this
		page: <a>https://www.vatsim.net/members/member-help.</a></p>
	<p>As a Division, we are unable to provide any details on why your account is suspended.</p>
@stop
